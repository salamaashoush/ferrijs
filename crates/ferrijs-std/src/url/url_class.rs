...
impl<'js> URL<'js> {
    ...
    /// instance. Used by callers (e.g. `crate::fetch`) that just need to know
...
/// `decodeURIComponent` over one URL component, which is what Node applies
/// to the credentials before joining them into `auth`. Invalid UTF-8 in the
/// decoded bytes is replaced rather than thrown, since the value came out of
...
pub fn url_to_http_options<'js>(ctx: Ctx<'js>, url: Class<'js, URL<'js>>) -> Result<Object<'js>> {
    ...
    // Node hands `http.request` a bare IPv6 host, not the URL's bracketed
    // form (`[::1]` -> `::1`), because that is what a socket connect takes.
    obj.set(
        "hostname",
        hostname
            .strip_prefix('[')
    ...
    if !port.is_empty() {
        // Node reports the port as a NUMBER; `http.request` compares it
        // numerically when it decides whether to append `:port` to Host.
