    ...
    pub fn new(ctx: Ctx<'js>, init: Opt<Value<'js>>) -> Result<Self> {
        if let Some(init) = init.into_inner() {
            if init.is_string() {
                ...
                // WebIDL: the init union is not nullable, so anything that
                // is neither a sequence nor a record converts to USVString
                // — `new URLSearchParams(null)` parses the query "null"
                // and `new URLSearchParams(42)` the query "42", the way
                // every browser engine does.
                return Ok(Self::from_str(Coerced::<String>::from_js(&ctx, init)?.0));
            }
        }
