...
fn get_readable_default_controller<'js>(
    ...
    match &readable.controller {
        crate::stream_web::readable::ReadableStreamControllerClass::ReadableStreamDefaultController(c) => {
            Some(c.clone())
        },
        _ => None,
    }
...
pub(super) fn transform_stream_default_controller_terminate<'js>(
    ...
    transform_stream_error_writable_and_unblock_write(ctx.clone(), stream_class, error)?;
    Ok(())
}
...
pub(super) fn transform_stream_error<'js>(
    ...
    transform_stream_error_writable_and_unblock_write(ctx.clone(), stream_class, e)?;
    Ok(())
}
...
pub(super) fn transform_stream_error_writable_and_unblock_write<'js>(
    ...
    if let Some(writable) = writable {
        crate::stream_web::writable::writable_stream_error_if_needed(ctx, writable, e)?;
    }
...
pub(super) fn transform_stream_default_controller_perform_transform<'js>(
    ...
    let promise_primordials = crate::stream_web::utils::promise::PromisePrimordials::get(&ctx)?.clone();
    ...
    // TransformStreamDefaultControllerPerformTransform step 3: react to
    // transformPromise's rejection by erroring the stream, then rethrow.
    // Without this a `transform()` that throws (or returns a rejected
    // promise) leaves both sides live: the reader's pending read never
    // settles and the writable is never errored.
