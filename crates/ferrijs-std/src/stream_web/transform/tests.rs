...
async fn identity_passthrough() {
    test_async_with(|ctx| {
        crate::stream_web::init(&ctx).unwrap();
        Box::pin(async move {
            eval_async(
                &ctx,
                r#"
...
#[tokio::test]
async fn transform_error_rejects_write_without_leaking() {
    test_async_with(|ctx| {
        crate::stream_web::init(&ctx).unwrap();
        Box::pin(async move {
            eval_async(
                ...
                const writer = ts.writable.getWriter();
                let message = null;
                try { await writer.write("x"); } catch (e) { message = e.message; }
                if (message !== "boom") throw new Error("got: " + message);
            "#,
            )
            .unwrap()
            .into_future::<()>()
...
/// A `transform()` that THROWS (rather than calling `controller.error`)
/// must error the stream too, so a pending read rejects instead of
/// hanging forever.
#[tokio::test]
async fn thrown_transform_error_reaches_a_pending_read() {
    test_async_with(|ctx| {
        crate::stream_web::init(&ctx).unwrap();
        Box::pin(async move {
            eval_async(
                ...
                w.write("x").catch(() => {});
                let message = null;
                try { await r.read(); } catch (e) { message = e.message; }
                if (message !== "boom") throw new Error("got: " + message);
            "#,
            )
            .unwrap()
