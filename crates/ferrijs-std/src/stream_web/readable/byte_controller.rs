...
fn copy_data_block_bytes(
    ...
    let to_raw = to_block
        ...
        .or_throw(ctx)?;
    let to_slice = unsafe { std::slice::from_raw_parts_mut(to_raw.cast::<u8>().as_ptr(), to_raw.len()) };
    let from_raw = from_block
        .as_raw()
        .ok_or(ERROR_MSG_ARRAY_BUFFER_DETACHED)
        .or_throw(ctx)?;
    let from_slice = unsafe { std::slice::from_raw_parts(from_raw.cast::<u8>().as_ptr(), from_raw.len()) };
