...
use crate::crypto::provider::{modern, HybridKemVariant, MlKemVariant};
...
impl EncapsulationAlgorithm {
    ...
    fn encapsulate(
        self,
        public_key: &[u8],
    ) -> std::result::Result<(Vec<u8>, Vec<u8>), crate::crypto::provider::CryptoError> {
        match self {
    ...
    fn decapsulate(
        ...
        private_key: &[u8],
        ciphertext: &[u8],
    ) -> std::result::Result<Vec<u8>, crate::crypto::provider::CryptoError> {
        match self {
