...
fn sha3_digest(algorithm: &'static lc_digest::Algorithm, input: &[u8]) -> Vec<u8> {
    lc_digest::digest(algorithm, input).as_ref().to_vec()
}
...
pub fn subtle_digest<'js>(
    ...
    async move {
        ...
        let bytes = match algorithm {
            ...
            DigestAlgorithm::Sha3_256 => sha3_digest(&lc_digest::SHA3_256, &input),
            DigestAlgorithm::Sha3_384 => sha3_digest(&lc_digest::SHA3_384, &input),
            DigestAlgorithm::Sha3_512 => sha3_digest(&lc_digest::SHA3_512, &input),
    ...
    let algorithm = match name {
        DigestAlgorithmName::CShake(strength) => {
            ...
            if u64::from(output_length).div_ceil(8) * 8 > u64::from(u32::MAX) {
                return Err(crate::exceptions::DOMException::operation_error(
                    ctx,
...
fn allocate_digest_output(ctx: &Ctx<'_>, length: usize) -> Result<Vec<u8>> {
    try_allocate_digest_output(length).map_err(|_| {
        crate::exceptions::DOMException::operation_error(ctx, "Digest output allocation failed")
    })
}
