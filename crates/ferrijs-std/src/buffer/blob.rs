...
impl<'js> Blob<'js> {
    ...
    pub fn stream(&self, ctx: Ctx<'js>) -> Result<Value<'js>> {
        // Local delta: upstream captures `self.data` — a JS `ArrayBuffer` —
        // in this native closure. A native closure that holds a JS value
        // makes a cycle the collector cannot see, which trips
        // `JS_FreeRuntime`'s `list_empty(&rt->gc_obj_list)` assertion at
        // teardown in this runtime. The bytes are copied out instead and the
        // buffer is rebuilt inside the closure.
        ...
        let pull = PullAlgorithm::from_fn_once(
            move |ctx: Ctx<'js>, controller: ReadableStreamControllerClass<'js>| {
                ...
                let len = data.len();
                if len != 0 {
                    let buffer = ArrayBuffer::new(ctx.clone(), data.clone())?;
                    let view = shared_array_buffer_view(&ctx, &buffer, 0, len)?;
